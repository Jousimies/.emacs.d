...
** COMMENT menu bar
把当前 tab 所属的 group 显示在 menu-bar 之上。

#+begin_src emacs-lisp
  (defun add-tab-group-to-menu-bar (tab-group-name)
    "Adds a tab group with the given name to the menu bar."
    (let ((tab-group-keymap (make-sparse-keymap)))
      (define-key global-map [menu-bar tab-group-name] (cons tab-group-name tab-group-keymap))))

  (add-hook 'post-command-hook (lambda ()
                                     (add-tab-group-to-menu-bar
                                      (concat
                                       "𝚻𝚨𝚩 "
                                       (number-to-string (tab-bar--current-tab-index))
...
** org-agenda
转到使用 =org-gtd= 处理 GTD 任务，它的 ~org-gtd-engage~ 比我定制的 =org-agenda-custom-commands= 看起来更简洁。
#+begin_src emacs-lisp :tangle no
      ...
      (setq org-agenda-category-icon-alist
            `(("\\`gtd\\'"
              ...
              ("\\\cc\\\|[a-zA-z0-9]*"
               ...
               nil nil :ascent center)))))
#+end_src
使用 =C-f12= 打开 ~org-agenda~ 然后执行相应的操作比使用快捷键的方式心智负担小一些。
#+begin_src emacs-lisp :tangle no
  (defun my/org-agenda ()
    "Open my `org-agenda'.
...
#+begin_src emacs-lisp
  ...
  (with-eval-after-load 'evil
    (evil-define-key 'normal 'org-mode-map
      "zm" 'orb-note-actions))
#+end_src
*** COMMENT bib with denote
[[https://www.scss.tcd.ie/~sulimanm/posts/denote-bibliography.html][Managing a bibliography of BiBTeX entries with Denote]].
#+begin_src emacs-lisp
