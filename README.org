...
** Attention
本配置需要升级 Emacs 到版本 29. 否则 tab-bar 在显示 global-mode-string 内容时会出现夸行的问题。
* The Early Init File
...
#+begin_src emacs-lisp
  (global-set-key (kbd "C-c l l") 'powerthesaurus-lookup-dwim)
#+end_src
*** Smog
还没搞懂这个软件怎么用，这里是一些资料。
...
** Bookmark, ebuku
#+begin_src emacs-lisp
  (global-set-key (kbd "C-c t m") 'ebuku)
#+end_src
** Email
...
#+begin_src emacs-lisp
  (add-hook 'mu4e-index-updated-hook
            (defun new-mail-sound ()
              (shell-command "afplay /System/Library/Sounds/Blow.aiff")))
#+end_src
*** mu4e-column-faces
#+begin_src emacs-lisp
  (with-eval-after-load 'mu4e
