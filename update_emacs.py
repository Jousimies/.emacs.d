...
def generate_idle_feature_cache() -> bool:
    ...
    try:
        ...
        log(f"idle feature 发现失败: {err}", "ERROR")
        return False
    ...
    if (not system_type
            ...
            or not roots
            or not features
            or any(not name or not FEATURE_RE.fullmatch(name)
    ...
    # Preserve provision order while defending against unexpected duplicates.
    ...
    IDLE_FEATURE_CACHE.parent.mkdir(parents=True, exist_ok=True)
...
def verify_cache() -> bool:
    ...
    if ("my/idle-loader-generated-roots" not in idle_text
...
def run_tests(skip_startup: bool = False) -> bool:
    ...
    if skip_startup:
        return True
    if (not LOAD_PATH_CACHE.is_file()
            or not PACKAGE_AUTOLOADS.is_file()
            or not IDLE_FEATURE_CACHE.is_file()):
...
def main() -> int:
    ...
    parser.add_argument(
        "--cache",
        action="store_true",
        help="只生成 load-path、autoload 与 idle feature 缓存",
    )
