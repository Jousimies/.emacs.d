...
** COMMENT Packages
需要运行一次 [[fmt:verbatim][package-quickstart-refresh]]
#+begin_src emacs-lisp :tangle ~/.emacs.d/init.el
...
*** Font setting
[[fmt:verbatim][set-face-attribute]] 设置默认字体, [[fmt:verbatim][set-fontset-font]] 用于指定某些字符集使用特定的字体。

See: [[https://archive.casouri.cc/note/2019/emacs-%E5%AD%97%E4%BD%93%E4%B8%8E%E5%AD%97%E4%BD%93%E9%9B%86/index.html][Emacs，字体与字符集]]

Maple Mono Font: [[https://github.com/subframe7536/maple-font][subframe7536/maple-font: Maple Mono]]
#+begin_src emacs-lisp :tangle ~/.emacs.d/init.el
  ;; 设置中文字集
...
行内灰色幽灵补全，代替 [[fmt:verbatim][Corfu]]。
#+begin_src emacs-lisp :tangle ~/.emacs.d/init.el
...
#+end_src
** orderless
Emacs 31 使用 flex 无法使用中文拼音或者形码搜索中文，所以还是保持默认的配置吧。
#+begin_src emacs-lisp :tangle ~/.emacs.d/init.el
...
#+end_src
** Embark
[[fmt:verbatim][embark-dwim]] 很方便，有很多功能可以尝试。如在文件路径上直接 [[fmt:verbatim][M-.]] 可以替代 [[fmt:verbatim][find-file-at-point]]，结合 [[fmt:verbatim][xref-go-back]] 可以快速的跳转回之前的位置。
#+begin_src emacs-lisp :tangle ~/.emacs.d/init.el
  (use-package embark
...
使用 [[fmt:verbatim][delete-horizontal-space]]、[[fmt:verbatim][delete-blank-lines]]、[[fmt:verbatim][cycle-spacing]] 和 [[fmt:verbatim][delete-trailing-whitespace]] 替代 [[fmt:verbatim][hungry-delete]]。

截止 20251203，[[fmt:verbatim][hungry-delete]] 己很久没有更新。

默认的按键绑定：
- [[fmt:verbatim][M-\]] for [[fmt:verbatim][delete-horizontal-space]]
- [[fmt:verbatim][C-x C-o]] for [[fmt:verbatim][delete-blank-lines]]
- [[fmt:verbatim][M-SPC]] for [[fmt:verbatim][cycle-spacing]]
...
*** avy
[[fmt:verbatim][avy]] 可以进行屏幕可视范围的跳转，不管是哪个 window。我使用 [[fmt:verbatim][avy]] 替代窗口跳转如 [[fmt:verbatim][windmove]]，直接使用 [[fmt:verbatim][avy-goto-char-timer]] 即可。
#+begin_src emacs-lisp :tangle ~/.emacs.d/init.el
  (use-package avy
...
#+end_src
*** selected
在 org-mode 文件中选择文本区域后，执行 [[fmt:verbatim][hl-highlight-region]] 或者 [[fmt:verbatim][fmt-style-region]]，给文字增加样式。

如果需要修改样式，先 er/expand-region 然后再执行相应的修改命令即可。
#+begin_src emacs-lisp :tangle ~/.emacs.d/init.el
...
*** ibuffer mode
使用 [[fmt:verbatim][consult-buffer]] 切换 [[fmt:verbatim][buffer]]，不太需要使用 [[fmt:verbatim][previous-buffer]] 和 [[fmt:verbatim][next-buffer]] 命令。
#+begin_src emacs-lisp :tangle ~/.emacs.d/init.el
...
#+end_src
** Dired Management
=Dired= 可以通过正则匹配文件名来删除文件，具体的操作如下：​~%m~​进行文件的查找，然后使用​~t~​选中未匹配的项目，使用​~k~​隐藏它们，留下的就是欲删除的文件。
#+begin_src emacs-lisp :tangle ~/.emacs.d/init.el
...
**** image size info
[[fmt:verbatim][identify]] 是 [[fmt:verbatim][ImageMagick]] 提供的函数，需要终端安装。
#+begin_src emacs-lisp :tangle ~/.emacs.d/init.el
...
#+end_src
** Xref
对于 xref 的使用不了解。初步只知道可以用于查找定义，然后快速的返回来前的位置。
#+begin_src emacs-lisp :tangle ~/.emacs.d/init.el
...
#+end_src
*** COMMENT denote sections
#+begin_src emacs-lisp :tangle ~/.emacs.d/init.el
  (use-package denote-sections
    :load-path "~/.emacs.d/packages/denote-sections/" "~/.emacs.d/packages/universal-sidecar/"
    )
#+end_src
...
#+end_src
** LaTeX & Export
Latex 中制图可以参见：[[https://castel.dev/post/lecture-notes-2/][How I draw figures for my mathematical lecture notes using Inkscape | Gilles Castel]]。虽然我目前没有实践这种方式。
*** ox-latex
#+begin_src emacs-lisp :tangle ~/.emacs.d/init.el
...
#+begin_src emacs-lisp :tangle ~/.emacs.d/init.el
  ...
  (use-package saveplace-pdf-view
    ...
    :defer t)
#+end_src
*** COMMENT pdf-tools modify metadata
#+begin_src emacs-lisp :tangle ~/.emacs.d/init.el
...
#+end_src
*** COMMENT Open file with system default app
#+begin_src emacs-lisp :tangle ~/.emacs.d/init.el
  (defun my/open-file-with-system ()
    "用 Preview 打开 PDF 并关闭当前 buffer。"
    (interactive)
    (when buffer-file-name
      (call-process "open" nil 0 nil buffer-file-name)
      (kill-buffer)))

  (add-to-list 'auto-mode-alist
                 '("\\.\\(docx\\|xlsx\\|pdf\\)\\'" . my/open-file-with-system))
#+end_src

*** COMMENT Open pdf with system preview
pdf-tools 还是不太好用，结合系统的方式进行吧。
#+begin_src emacs-lisp :tangle ~/.emacs.d/init.el
...
See: [[https://gsj987.github.io/use-org-mode-publish-blog.html#org36c61a7][使用 org-mode 写博客]] and [[https://nicolasknoebber.com/posts/blogging-with-emacs-and-org.html#org9edef43][Blogging with Emacs and Org]]。
#+begin_src emacs-lisp :tangle ~/.emacs.d/init.el
...
Mac Tahoe 26.2 有一些变更导致会提示无权限写入日历。可以尝试重置日历权限或者终端执行以下命令：

给生成的 dylib 签名。
#+begin_src bash
  codesign -s - .build/release/liborg2calendar.dylib
#+end_src

给 Emacs 申请权限。经过测试，需要执行下面的命令获取权限才可以。

获取 Calendar 权限：
#+begin_src bash
  defaults write /Applications/Emacs.app/Contents/Info.plist NSCalendarsFullAccessUsageDescription "Org-mode needs calendar access to sync your clocks."
#+end_src
...
** Open file with system default applications
#+begin_src emacs-lisp :tangle ~/.emacs.d/init.el
...
** COMMENT Open mp4 file with mpv
#+begin_src emacs-lisp :tangle ~/.emacs.d/init.el
